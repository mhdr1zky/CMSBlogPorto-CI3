...
class Blog extends CI_Controller {
	...
	function detail($slug){
		...
		if($data->num_rows() > 0){
    		...
    		$w['related_post']  = $this->blog_model->get_related_post($category_id,$kode);
    		$x['show_comments'] = $this->blog_model->show_comments($kode);
    		$site_info = $this->db->get('tbl_site', 1)->row();
			$v['logo'] =  $site_info->site_logo_header;
			$x['icon'] = $site_info->site_favicon;
			$w['populer_post'] = $this->blog_model->get_popular_post();
			$x['related_post'] = $this->load->view('v_related_post',$w,TRUE);
    		$x['header'] = $this->load->view('header',$v,TRUE);
    		$x['footer'] = $this->load->view('footer','',TRUE);
    		$site = $this->site_model->get_site_data()->row_array();
			$x['site_name'] = $site['site_name'];
